...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMy_Costom_SvcHdle;                    /**< MY_COSTOM_SVC handle */
 ...
 */
#define COPY_MY_COSTOM_SVC_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for MY_COSTOM_SVC +
   *                                2 for MY_CHAR +
   *                              = 3
   */

  COPY_MY_COSTOM_SVC_UUID(uuid.Char_UUID_128);
  aci_gatt_add_service(UUID_TYPE_128,
                       (Service_UUID_t *) &uuid,
                       PRIMARY_SERVICE,
                       3,
                       &(CustomContext.CustomMy_Costom_SvcHdle));

  /**
   *  MY_CHAR
   */
  COPY_MY_CHAR_UUID(uuid.Char_UUID_128);
  aci_gatt_add_char(CustomContext.CustomMy_Costom_SvcHdle,
                    UUID_TYPE_128, &uuid,
                    SizeMy_Char,
                    CHAR_PROP_READ,
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch(CharOpcode)
  {
    ...
    case CUSTOM_STM_MY_CHAR:
      result = aci_gatt_update_char_value(CustomContext.CustomMy_Costom_SvcHdle,
                                          CustomContext.CustomMy_CharHdle,
                                          0, /* charValOffset */
